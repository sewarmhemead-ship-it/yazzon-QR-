...
function frame(){
  ...
  meshes.forEach(({unit,mesh,frame,reflection},index)=>{
    ...
    const nextPeek=mobile&&d>0?.24:0;
    const x=galleryOffset+Math.sin(angle)*(mobile?3.25:4.9)-nextPeek;
    ...
    const peekScale=mobile?(d>0?1.08:d<0?.94:1):1;
    const scale=baseScale*Math.max(.58,1-Math.abs(d)*.18)*peekScale; unit.scale.setScalar(scale);
    const sideLight=mobile?(d>0?1.2:d<0?.68:1):1;
    mesh.material.opacity=Math.min(1,Math.max(.16,1-Math.abs(d)*.3)*sideLight);
    frame.material.opacity=Math.min(.96,Math.max(.34,.96-Math.abs(d)*.22)*sideLight);
